...
function (network::GapNN)(args...; kwargs...)
    return softplus(network._nn(args...; kwargs...))
end
...
function composite_loss_new(model::LooslyCoupledModel, x, u, f)
    ...
    pred_gap = nn(x, u)
    pred = model(x, u)
    l_minorant = 1e+1 * loss_minorant(pcm, x, u, f)
    l_mse = 1e-0 * Flux.Losses.mse(pred, f)
    l_tight_gap = 1e-1 * Flux.Losses.mse(pred_gap, 0.0)
...
function main(
    ...
    function callback(network, epoch)
        ...
        elseif model_name == :cplse
            ...
            @show l_nonnegativity_violation =
                get_loss(network.nn, dataset[:test], loss_nonnegativity_violation)
        elseif model_name == :plse
            @show l_mse = get_loss(network, dataset[:test], loss_mse)
            @show l_minorant = get_loss(network, dataset[:test], loss_minorant)
        ...
        # contour
        fig_ctr =
            plot(; title = "contour (iter: $epoch/$epochs)", xlabel = "c", ylabel = "d")
        plot!(
            fig_ctr,
            c_plot,
            d_plot,
        ...
        if model_name == :eplse
            plot!(
                ...
                cs_ctr,
                hcat(
                    [minimise(network, [c]; min_decision, max_decision) for c in cs_ctr]...,
                )';
                label = "solution by pcm",
            )
        elseif model_name == :cplse
            ...
            initial_guesses =
                [minimise(network.pcm, [c]; min_decision, max_decision) for c in cs_ctr]
            plot!(
                fig_ctr,
                cs_ctr,
                hcat(initial_guesses...)';
                lw = 2.0,
                label = "solution by pcm (initial guess)",
            )
            plot!(
                fig_ctr,
                cs_ctr,
                hcat(
                    [
                        minimise_nlopt(
                            opt,
                            d -> target_function([c], d),
                            min_decision,
                            max_decision,
                            initial_guess,
                        ) for (c, initial_guess) in zip(cs_ctr, initial_guesses)
                    ]...,
                )';
                lw = 2.0,
                label = "solution with NLopt",
            )
        elseif model_name == :plse
            plot!(
                fig_ctr,
        ...
        elseif model_name == :cplse
            ...
            plot!(
                fig_loss,
                1:length(ls_nonnegativity_violation),
                ls_nonnegativity_violation;
                label = "Nonnegativity violation",
            )
