...
function DecisionMakingDataset(
    ...
    metadata = (;
        metadata...,
        name = name,
        split_ratio = (; train = ratio1, validate = ratio2, test = 1 - (ratio1 + ratio2)),
        train_idx = train_idx,
...
function Base.getindex(dataset::DecisionMakingDataset, split)
    ...
    if split == :full
        ...
        dataset_ = DecisionMakingDataset(
            metadata,
            split,
            conditions[idx],
            decisions[idx],
...
function split_data3(dataset, ratio1, ratio2; rng = Random.default_rng())
    ...
    dataset_validate, dataset_test =
        split_data2(dataset_valtest, ratio2 / (1.0 - ratio1); rng)
...
function sample_from_bounds(N, min_value, max_value; rng = Random.default_rng())
    ...
    for i = 1:N
        sampled_value =
            (min_value + (max_value - min_value) .* rand(rng, size(min_value)...))
